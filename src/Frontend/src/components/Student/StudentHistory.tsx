...
export const StudentHistory = ({student}:{student:Student}) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-500 shadow-lg rounded-xl flex flex-col overflow-y-hidden">
            <h1 className="text-gray-700 dark:text-white py-5 px-5 font-bold text-2xl">History</h1>
...
const HistoryEntry = ({history}: { history: EventHistory }) => {
    ...
    return (
        <li className="p-5 grid grid-cols-4">
            <img className="dark:text-white px-4 col-span-1 object-contain transform w-24" src="img/fallingdown.png" alt={history.event}/>
            <h1 className="dark:text-white text-xl row-span-2 col-span-2">{GetUrgentEventMessage(history.student!, history.event!)}</h1>
            <TimeAgo className="dark:text-white row-span-2 col-span-1 text-right pr-4" date={history.date!}/>
        </li>)
}
